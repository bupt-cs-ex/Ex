...
错误率0.0001、0.00001、0.000001的运行时间之和为 3792 ms

#### 5.2.3 内存峰值

错误率0.0001、0.00001、0.000001下的内存峰值分别为3MB、7MB、11MB
...
| 成员              | 分工                 |
| ----------------- | -------------------- |
| 王尚荣 2020110636 | bloom filter         |
| 王宝江 2020110630 | hashtable 字符串检索 |
...
- bloom filter 使用 k 个相互独立的哈希函数将每一个 keyword 映射到 二进制位数组的 k 位置，并将该 k 个位置置为 1 ，查找时使用相同的 k 个哈希函数将 keyword 映射到 k 个位置，若 k 个位置都为1，则认为查找成功，否则查找失败。
